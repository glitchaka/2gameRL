...
final class Studio221ResourcesTest {
    @Test void canonicalBibleIsBundledAndSubstantial() throws Exception {
        try(InputStream in=getClass().getResourceAsStream("/manuals/2GAMESCRIPT_BIBLE.md")){
            ...
            for(String required:new String[]{"Biblia de 2GameScript 2.2.1","PlatformerController","Rigidbody2D.grounded","spawnPrefab","showText","Gramática práctica","Límites explícitos"})assertTrue(text.contains(required),required);
        }
    }
