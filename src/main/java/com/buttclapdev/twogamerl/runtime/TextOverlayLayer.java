...
final class TextOverlayLayer extends Pane {
    ...
    private final List<Entry>entries=new ArrayList<>();private final Function<String,Point2D>anchorResolver;
    TextOverlayLayer(Function<String,Point2D>anchorResolver){this.anchorResolver=anchorResolver;setPickOnBounds(false);}
    void show(TextRequest raw,double now){if(raw==null)return;String type=raw.type()==null?"free":raw.type().toLowerCase(Locale.ROOT);double duration=raw.duration();if(type.equals("bubble"))duration=Math.max(.25,Math.min(10,duration<=0?3:duration));else if(type.equals("free"))duration=Math.max(.25,duration<=0?3:duration);else if(type.equals("novel")&&raw.dismiss().equalsIgnoreCase("time"))duration=Math.max(.25,duration);Node node=switch(type){case"bubble"->bubble(raw.text());case"novel"->novel(raw.anchor(),raw.text());default->free(raw.text());};Entry e=new Entry(type,raw.anchor()==null?"screen":raw.anchor(),raw.dismiss()==null?"time":raw.dismiss().toLowerCase(Locale.ROOT),raw.key()==null?"":raw.key(),duration,now,node);entries.add(e);getChildren().add(node);layoutEntry(e);node.toFront();}
    void update(double now){for(Iterator<Entry>it=entries.iterator();it.hasNext();){Entry e=it.next();layoutEntry(e);double age=now-e.started;if(!e.dismissed&&e.dismiss.equals("time")&&e.duration>0&&age>=e.duration)e.dismiss(now);if(!e.dismissed&&(e.type.equals("free")||e.type.equals("bubble"))&&e.duration>0){double remaining=e.duration-age,fadeIn=Math.min(1,age/.15),fadeOut=Math.min(1,Math.max(0,remaining)/FADE);e.node.setOpacity(Math.min(fadeIn,fadeOut));}else if(e.dismissed){double p=1-(now-e.dismissedAt)/FADE;e.node.setOpacity(Math.max(0,p));if(p<=0){getChildren().remove(e.node);it.remove();}}}}
    void click(double now){for(Entry e:entries)if(e.type.equals("novel")&&e.dismiss.equals("click")&&!e.dismissed)e.dismiss(now);}void key(KeyCode code,double now){for(Entry e:entries)if(e.type.equals("novel")&&e.dismiss.equals("key")&&!e.dismissed&&(e.key.isBlank()||code.name().equalsIgnoreCase(e.key)))e.dismiss(now);}void clear(){entries.clear();getChildren().clear();}int count(){return entries.size();}
