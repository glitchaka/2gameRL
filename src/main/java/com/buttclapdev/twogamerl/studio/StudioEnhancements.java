...
final class StudioEnhancements {
    ...
    static void install(Stage stage,StudioApp app){ThemeManager.install();ThemeManager.apply(stage.getScene());installMenus(stage,app);replacePreviewButton(stage.getScene().getRoot(),app);installWorkspaceHooks(stage,app);StudioControlPolish.install(stage.getScene().getRoot());installScriptEditors(stage.getScene().getRoot(),app);TutorialPatch222.install(stage.getScene().getRoot(),stage);installPreviewCloseHook();}
    ...
    private static void installMenus(Stage stage,StudioApp app){
        ...
        bar.getMenus().removeIf(m->"RC 2.3".equals(m.getText()));
