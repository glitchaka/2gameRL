...
final class StudioEnhancements {
    ...
    private static void installMenus(Stage stage,StudioApp app){
        ...
        if(bar.getMenus().stream().noneMatch(m->"Recursos".equals(m.getText()))){Menu resources=new Menu("Recursos");MenuItem browser=new MenuItem("Navegador global de recursos…");browser.setAccelerator(KeyCombination.keyCombination("Ctrl+Shift+R"));browser.setOnAction(e->ResourceBrowser222.show(stage,app));resources.getItems().add(browser);int helpIndex=-1;for(int i=0;i<bar.getMenus().size();i++)if("Ayuda".equals(bar.getMenus().get(i).getText())){helpIndex=i;break;}if(helpIndex<0)bar.getMenus().add(resources);else bar.getMenus().add(helpIndex,resources);}
