...
final class GraphicsBrowserEnhancements {
    ...
    private static void installImageViews(VBox imageBox,ListView<Asset>list,StudioApp app){
        ...
        Runnable rebuild=()->{removeInternal(list);rebuildAssetTree(tree,list.getItems(),app);};attachListListener(list,rebuild);rebuild.run();
    ...
    private static void installGlobalBrowser(GraphicsEditorPane pane,StudioApp app){
        ...
        tree.getSelectionModel().selectedItemProperty().addListener((o,a,b)->{if(b!=null&&b.getValue()!=null)route(pane,b.getValue());});flat.getSelectionModel().selectedItemProperty().addListener((o,a,b)->{if(b!=null)route(pane,b);});
        ...
        leftTabs.getTabs().add(0,new Tab("Todos los recursos",box));leftTabs.getSelectionModel().selectFirst();pane.getProperties().put("2rl.resourceBrowser.refresh",rebuild);
    }
    ...
    private static void rebuildResources(StudioApp app,String raw,TreeView<ResourceItem>tree,ListView<ResourceItem>flat){
        ...
        addGroup(groups,"Imágenes",app.project().getAssets().values().stream().filter(a->!ParticleVisualMeta.internal(a)).map(a->new ResourceItem(assetKind(a),a.sourceName==null?a.key:a.sourceName,assetKind(a),a)).toList());
        addGroup(groups,"Fuentes",app.project().getFonts().values().stream().map(f->new ResourceItem("Fuente",f.sourceName,"Fuente",f)).toList());
        addGroup(groups,"Pinceles / Prefabs",app.project().getPrefabs().values().stream().map(p->new ResourceItem("Prefab",p.name,p.placement.name(),p)).toList());
        addGroup(groups,"Animation Clips",app.project().getAnimationClips().values().stream().map(c->new ResourceItem("AnimationClip",c.name,c.frames.size()+" frames",c)).toList());
        addGroup(groups,"Animator Controllers",app.project().getAnimatorControllers().values().stream().map(c->new ResourceItem("AnimatorController",c.name,c.states.size()+" estados",c)).toList());
        addGroup(groups,"Partículas",unique(app.project().getParticlePresets().values()).stream().map(p->new ResourceItem("ParticlePreset",p.name,p.burst?"Burst":"Continuo",p)).toList());
        addGroup(groups,"Paletas",unique(app.project().getPalettes().values()).stream().map(p->new ResourceItem("Paleta",p.name,p.colors.size()+" colores",p)).toList());
        addGroup(groups,"TextStyles",unique(app.project().getTextStyles().values()).stream().map(s->new ResourceItem("TextStyle",s.name,s.fontSize+" px",s)).toList());
    ...
    private static void route(GraphicsEditorPane pane,ResourceItem item){if(item==null||item.value()==null)return;Object v=item.value();String field=switch(v){case Asset a->"assets";case FontAsset f->"fonts";case PrefabDef p->"prefabs";case AnimationClip c->"clips";case AnimatorController c->"controllers";case ParticlePreset p->"particles";case PaletteAsset p->"palettes";case TextStyle s->"textStyles";default->"";};if(field.isBlank())return;ListView<?>target=field(pane,field,ListView.class);if(target!=null){@SuppressWarnings({"rawtypes","unchecked"})ListView raw=target;raw.getSelectionModel().clearSelection();raw.getSelectionModel().select(v);raw.scrollTo(v);}if(v instanceof Asset||v instanceof FontAsset){TabPane left=findFirst(pane.getItems().getFirst(),TabPane.class);if(left!=null)selectTab(left,v instanceof FontAsset?"Fuentes":"Imágenes");}else{TabPane right=findFirst(pane.getItems().getLast(),TabPane.class);if(right!=null)selectTab(right,v instanceof PrefabDef?"Pinceles / Prefabs":v instanceof AnimationClip||v instanceof AnimatorController?"Animación":v instanceof TextStyle?"Textos":"Efectos");}}
