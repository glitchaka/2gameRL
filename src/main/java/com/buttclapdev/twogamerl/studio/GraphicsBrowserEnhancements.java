...
final class GraphicsBrowserEnhancements {
    ...
    private static void rebuildTree(TreeView<Object>tree,List<Asset>assets,StudioApp app){
        TreeItem<Object>root=new TreeItem<>("Assets");root.setExpanded(true);
        LinkedHashMap<String,TreeItem<Object>>categories=new LinkedHashMap<>();
        for(String name:List.of("Imágenes","Spritesheets","Sprites","Fondos","UI","Tilesets","VFX","Retratos")){TreeItem<Object>folder=new TreeItem<>(name);folder.setExpanded(true);categories.put(name,folder);root.getChildren().add(folder);}
        Map<String,TreeItem<Object>>sourceFolders=new LinkedHashMap<>();
        for(Asset asset:assets){String category=categoryName(asset);TreeItem<Object>folder=categories.get(category);if(folder==null)folder=categories.get("Imágenes");if(asset.isRegion()){
                String sourceKey=asset.sourceAssetKey;String id=category+"/"+sourceKey;TreeItem<Object>sourceFolder=sourceFolders.get(id);if(sourceFolder==null){Asset source=app.project().getAssets().get(sourceKey);String display=source==null?sourceKey:(source.sourceName==null?source.key:source.sourceName);sourceFolder=new TreeItem<>(display);sourceFolder.setExpanded(true);folder.getChildren().add(sourceFolder);sourceFolders.put(id,sourceFolder);}sourceFolder.getChildren().add(new TreeItem<>(asset));
            }else folder.getChildren().add(new TreeItem<>(asset));}
        root.getChildren().removeIf(f->f.getChildren().isEmpty());tree.setRoot(root);
    }
