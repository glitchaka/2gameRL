...
final class AssetExplorerPane extends BorderPane {
    ...
    private Node assetTile(Asset asset){
        StackPane thumb=thumbnail(asset,86,62);Label label=new Label(displayName(asset));label.setWrapText(true);label.setMaxWidth(118);label.getStyleClass().add("resource-file-name");Label type=new Label(typeName(asset));type.getStyleClass().add("resource-file-type");VBox box=new VBox(6,thumb,label,type);box.setAlignment(Pos.TOP_LEFT);box.setPrefSize(132,126);box.getStyleClass().add("resource-asset-tile");if(selectedKeys.contains(asset.key))box.getStyleClass().add("selected");
    ...
    private void rebuildBreadcrumbs(){breadcrumbs.getChildren().clear();Button root=breadcrumb("Assets","");breadcrumbs.getChildren().add(root);if(favoritesMode){breadcrumbs.getChildren().addAll(new Label("›"),breadcrumb("Favoritos",""){{setOnAction(e->{favoritesMode=true;rebuild();});}});return;}String path="";for(String part:currentFolder.split("/")){if(part.isBlank())continue;breadcrumbs.getChildren().add(new Label("›"));path=path.isBlank()?part:path+"/"+part;breadcrumbs.getChildren().add(breadcrumb(part,path));}}
