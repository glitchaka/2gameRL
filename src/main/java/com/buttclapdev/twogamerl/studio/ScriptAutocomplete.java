...
final class ScriptAutocomplete {
    ...
    private static final List<String> COMMANDS=List.of("log","print","move","velocity","teleport","bounce","destroy","destroyEntity","wait","timer","every","create","spawn","spawnPrefab","playAnimation","queueAnimation","stopAnimation","loadScene","showMenu","restartScene","setSprite","setEntitySprite","emit","showText","setVar","addVar","mulVar","divVar","randomVar","setGlobal","addGlobal","set","setEntity","moveEntity","teleportEntity","setEntityVar","addEntityVar","addComponent","removeComponent","setComponent","enableComponent","damage","heal","ifKey","ifPressed","ifAction","ifActionPressed","ifVar","ifGlobal","ifProperty","ifEntity","ifComponent","ifOther","chance","repeat","stop","return","else","end");
    ...
    private final Supplier<Collection<String>> localVariables;
    ...
    private void install(){
        ...
        editor.addEventFilter(KeyEvent.KEY_RELEASED,e->{if(e.isControlDown()||e.isAltDown()||e.isMetaDown())return;KeyCode k=e.getCode();if(k.isLetterKey()||k.isDigitKey()||k==KeyCode.PERIOD||k==KeyCode.MINUS||k==KeyCode.UNDERSCORE)refresh(false);});
        editor.focusedProperty().addListener((o,a,b)->{if(!b)menu.hide();});
    }

    private void refresh(boolean forced){
        int caret=editor.getCaretPosition();String text=editor.getText();if(caret<0||caret>text.length())return;int lineStart=text.lastIndexOf('\n',Math.max(0,caret-1))+1;String before=text.substring(lineStart,caret);int tokenStart=caret;while(tokenStart>lineStart&&isTokenChar(text.charAt(tokenStart-1)))tokenStart--;String prefix=text.substring(tokenStart,caret);if(!forced&&prefix.length()<1){menu.hide();return;}
        List<Suggestion>suggestions=suggestions(before,prefix);if(suggestions.isEmpty()){menu.hide();return;}show(suggestions,tokenStart,caret);
    }

    private List<Suggestion> suggestions(String lineBefore,String prefix){
        String trimmed=lineBefore.stripLeading();String lower=trimmed.toLowerCase(Locale.ROOT);String p=prefix.toLowerCase(Locale.ROOT);LinkedHashMap<String,Suggestion>out=new LinkedHashMap<>();
        if(lower.matches("on\\s+[^ ]*$")){for(String e:EVENTS)add(out,e,e,"Evento",p);return limit(out);}
        if(afterCommand(lower,"loadscene")){addProjectIds(out,app.project().getLevels().values().stream().map(l->new String[]{l.id,l.name}).toList(),"Escena",p);return limit(out);}
        if(afterCommand(lower,"showmenu")){addProjectIds(out,app.project().getMenus().values().stream().map(m->new String[]{m.id,m.title}).toList(),"Menú",p);return limit(out);}
        ...
        if(afterCommand(lower,"emit")){for(String e:knownNamedEvents())add(out,e,e,"Señal",p);return limit(out);}
    ...
    private Collection<String>knownLocals(){LinkedHashSet<String>r=new LinkedHashSet<>(localVariables.get());Matcher m=LOCAL.matcher(editor.getText());while(m.find())r.add(m.group(1));return r;}
    private Collection<String>knownGlobals(){return scanScripts(GLOBAL);}
    private Collection<String>knownSaves(){return scanScripts(SAVE);}
    private Collection<String>knownNamedEvents(){LinkedHashSet<String>r=new LinkedHashSet<>();for(String s:allScripts()){Matcher m=NAMED_EVENT.matcher(s);while(m.find())r.add(m.group(1));}return r;}
    ...
    private static void addProjectIds(Map<String,Suggestion>out,List<String[]>values,String group,String prefix){for(String[]v:values)add(out,v[0],v[0]+" — "+v[1],group,prefix);}
