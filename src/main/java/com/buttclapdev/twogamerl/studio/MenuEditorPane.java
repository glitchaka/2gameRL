...
final class MenuEditorPane extends SplitPane {
    ...
    private Node left() {
        ...
        Button addButton = new Button("＋ Botón");
    ...
    private Node center() {
        ...
        canvas.setOnMousePressed(e -> {
            if (e.getTarget() == canvas) { selection = Selection.SCREEN; selectedButton = null; rebuildInspector(); render(); }
        });
        StackPane shell = new StackPane(canvas); shell.setPadding(new Insets(35)); shell.getStyleClass().add("canvas-shell");
        ScrollPane scroll = new ScrollPane(shell); scroll.setFitToWidth(true); scroll.setFitToHeight(true); scroll.setPannable(true);
        return scroll;
    }

    private Node right() {
        ScrollPane scroll = new ScrollPane(inspector); scroll.setFitToWidth(true); scroll.getStyleClass().add("editor-scroll");
        inspector.getStyleClass().add("side-panel"); inspector.setPadding(new Insets(12)); inspector.setPrefWidth(390);
        return scroll;
    }
    ...
    private void refresh() {
        ...
        if (!picker.getItems().isEmpty()) {
            MenuScreen start = app.project().getMenus().get(app.project().getStartMenu());
            picker.setValue(start == null ? picker.getItems().getFirst() : start);
        }
        selection = Selection.SCREEN; selectedButton = null; render(); rebuildInspector();
    }

    private void render() {
        MenuScreen m = picker.getValue();
        ...
        java.awt.Color bg = m.background;
        canvas.setStyle(String.format(Locale.ROOT, "-fx-background-color: rgb(%d,%d,%d);", bg.getRed(), bg.getGreen(), bg.getBlue()));

        if (!m.backgroundAssetKey.isBlank()) {
            Image image = app.image(m.backgroundAssetKey);
            if (image != null) {
                ImageView background = new ImageView(image); background.setSmooth(false); background.setPreserveRatio(false);
                background.setFitWidth(m.canvasWidth); background.setFitHeight(m.canvasHeight); background.setMouseTransparent(true);
                canvas.getChildren().add(background);
            }
        }

        Node titleNode = buildTitleNode(m);
        titleNode.setLayoutX(m.titleX); titleNode.setLayoutY(m.titleY);
        installTitleDrag(titleNode, m);
        canvas.getChildren().add(titleNode);

        for (MenuButton def : m.buttons) {
            Node node = buildButtonNode(def);
            node.setLayoutX(def.x); node.setLayoutY(def.y);
            installButtonDrag(node, def, m);
            canvas.getChildren().add(node);
        }
    }

    private Node buildTitleNode(MenuScreen m) {
        StackPane content = new StackPane(); content.setPrefSize(m.titleWidth, m.titleHeight); content.setMinSize(m.titleWidth, m.titleHeight); content.setMaxSize(m.titleWidth, m.titleHeight);
        if (!m.titleAssetKey.isBlank()) {
            Image image = app.image(m.titleAssetKey);
            if (image != null) { ImageView iv = new ImageView(image); iv.setSmooth(false); iv.setPreserveRatio(false); iv.setFitWidth(m.titleWidth); iv.setFitHeight(m.titleHeight); content.getChildren().add(iv); }
        }
        if (m.title != null && !m.title.isBlank()) {
            Label label = new Label(m.title); label.setFont(Font.font(Math.max(8, m.titleFontSize))); label.setTextFill(fx(m.titleColor)); label.setWrapText(true); label.setAlignment(Pos.CENTER); label.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE); content.getChildren().add(label);
        }
        if (selection == Selection.TITLE) content.setStyle("-fx-border-color:#52d3ff;-fx-border-width:2;");
        return MenuEffects.decorate(content, m.titleAnimation, m.titleAnimationSpeed, MenuHoverEffect.NONE);
    }

    private Node buildButtonNode(MenuButton def) {
        ...
        ImageView imageView = new ImageView(); imageView.setSmooth(false); imageView.setPreserveRatio(false); imageView.setFitWidth(def.width); imageView.setFitHeight(def.height);
        Image normal = def.assetKey.isBlank() ? null : app.image(def.assetKey);
        Image hover = def.hoverAssetKey.isBlank() ? null : app.image(def.hoverAssetKey);
        if (normal != null) { imageView.setImage(normal); content.getChildren().add(imageView); }
        else {
        ...
        if (normal != null && hover != null) {
            content.setOnMouseEntered(e -> imageView.setImage(hover)); content.setOnMouseExited(e -> imageView.setImage(normal));
        }
        if (selection == Selection.BUTTON && selectedButton == def) content.setStyle(content.getStyle() + ";-fx-border-color:#52d3ff;-fx-border-width:2;");
        return MenuEffects.decorate(content, def.animation, def.animationSpeed, def.hoverEffect);
    }

    private void installTitleDrag(Node node, MenuScreen m) {
        double[] drag = new double[2];
        node.setOnMousePressed(e -> { selection=Selection.TITLE;selectedButton=null;drag[0]=e.getSceneX()-m.titleX;drag[1]=e.getSceneY()-m.titleY;rebuildInspector();render();e.consume(); });
    ...
    private void installButtonDrag(Node node, MenuButton def, MenuScreen m) {
        double[] offset = new double[2];
        node.setOnMousePressed(e -> { selection=Selection.BUTTON;selectedButton=def;offset[0]=e.getX();offset[1]=e.getY();rebuildInspector();render();e.consume(); });
        node.setOnMouseDragged(e -> { double baseX=canvas.localToScene(0,0).getX(),baseY=canvas.localToScene(0,0).getY();def.x=clamp((int)Math.round(e.getSceneX()-baseX-offset[0]),0,m.canvasWidth-def.width);def.y=clamp((int)Math.round(e.getSceneY()-baseY-offset[1]),0,m.canvasHeight-def.height);app.changed();render();e.consume(); });
    }

    private void rebuildInspector() {
        inspector.getChildren().clear(); MenuScreen m = picker.getValue(); if (m == null) { inspector.getChildren().add(new Label("No hay menú.")); return; }
        switch (selection) {
            case SCREEN -> screenInspector(m);
            case TITLE -> titleInspector(m);
            case BUTTON -> buttonInspector(m, selectedButton);
        }
    }

    private void screenInspector(MenuScreen m) {
        TextField screenName = new TextField(m.title);
        ColorPicker background = pickerColor(m.background);
        ComboBox<String> backgroundSprite = assetPicker(m.backgroundAssetKey);
        TextField width = new TextField(Integer.toString(m.canvasWidth)), height = new TextField(Integer.toString(m.canvasHeight));
        CheckBox initial = new CheckBox("Pantalla inicial"); initial.setSelected(m.id.equals(app.project().getStartMenu()));
        Runnable apply = () -> {
        ...
        inspector.getChildren().addAll(title("PANTALLA · "+m.id),form,initial,hint("El sprite de fondo se escala al tamaño de la pantalla. Haz clic en el título para editarlo como elemento visual independiente."));
    }
    ...
    private void buttonInspector(MenuScreen m, MenuButton b) {
        ...
        TextField text=new TextField(b.text),x=num(b.x),y=num(b.y),w=num(b.width),h=num(b.height),font=num(b.fontSize),target=new TextField(b.target),speed=new TextField(format(b.animationSpeed));
        ComboBox<MenuAction> action=new ComboBox<>(FXCollections.observableArrayList(MenuAction.values()));action.setValue(b.action);
        ComboBox<String> sprite=assetPicker(b.assetKey),hoverSprite=assetPicker(b.hoverAssetKey);
        ColorPicker textColor=pickerColor(b.textColor),bgColor=pickerColor(b.backgroundColor);
        ComboBox<MenuAnimation> animation=new ComboBox<>(FXCollections.observableArrayList(MenuAnimation.values()));animation.setValue(b.animation);
        ComboBox<MenuHoverEffect> hover=new ComboBox<>(FXCollections.observableArrayList(MenuHoverEffect.values()));hover.setValue(b.hoverEffect);
        Runnable apply=()->{b.text=text.getText();b.x=parseInt(x,b.x);b.y=parseInt(y,b.y);b.width=Math.max(1,parseInt(w,b.width));b.height=Math.max(1,parseInt(h,b.height));b.fontSize=Math.max(8,parseInt(font,b.fontSize));b.action=action.getValue();b.target=target.getText();b.assetKey=assetValue(sprite);b.hoverAssetKey=assetValue(hoverSprite);b.textColor=awt(textColor.getValue());b.backgroundColor=awt(bgColor.getValue());b.animation=animation.getValue();b.hoverEffect=hover.getValue();b.animationSpeed=Math.max(.2,parseDouble(speed,b.animationSpeed));app.changed();render();};
        for(TextField f:new TextField[]{text,x,y,w,h,font,target,speed})bind(f,apply);action.setOnAction(e->apply.run());sprite.setOnAction(e->apply.run());hoverSprite.setOnAction(e->apply.run());textColor.setOnAction(e->apply.run());bgColor.setOnAction(e->apply.run());animation.setOnAction(e->apply.run());hover.setOnAction(e->apply.run());
        Button delete=new Button("Eliminar botón");delete.getStyleClass().add("danger-button");delete.setOnAction(e->{m.buttons.remove(b);selectedButton=null;selection=Selection.SCREEN;app.changed();render();rebuildInspector();});
        GridPane form=form();row(form,0,"Texto",text);row(form,1,"X",x);row(form,2,"Y",y);row(form,3,"Ancho",w);row(form,4,"Alto",h);row(form,5,"Fuente",font);row(form,6,"Color texto",textColor);row(form,7,"Color base",bgColor);row(form,8,"Sprite",sprite);row(form,9,"Sprite hover",hoverSprite);row(form,10,"Acción",action);row(form,11,"Destino",target);row(form,12,"Animación",animation);row(form,13,"Hover",hover);row(form,14,"Velocidad",speed);
        inspector.getChildren().addAll(title("BOTÓN"),form,delete,hint("OPEN_MENU usa el ID de otra pantalla. Sprite hover es opcional; los efectos SCALE/GLOW/LIFT se ejecutan al pasar el mouse."));
    }
    ...
    private void removeMenu(){MenuScreen m=picker.getValue();if(m==null||app.project().getMenus().size()<=1)return;app.project().getMenus().remove(m.id);if(m.id.equals(app.project().getStartMenu()))app.project().setStartMenu(app.project().getMenus().keySet().iterator().next());app.changed();refresh();}
    private void addButton(){MenuScreen m=picker.getValue();if(m==null)return;MenuButton b=new MenuButton("Botón",Math.max(0,m.canvasWidth/2-100),Math.max(0,m.canvasHeight/2-24),200,48,MenuAction.START_GAME,"");m.buttons.add(b);selectedButton=b;selection=Selection.BUTTON;app.changed();render();rebuildInspector();}
