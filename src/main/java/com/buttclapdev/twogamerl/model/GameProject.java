...
public final class GameProject {
    ...
    public GameProject deepCopy(){
        ...
        c.tiles.clear();tiles.forEach((k,v)->c.tiles.put(k,v.copy()));c.levels.clear();levels.forEach((k,v)->c.levels.put(k,v.copy()));c.menus.clear();menus.forEach((k,v)->c.menus.put(k,v.copy()));c.assets.clear();assets.forEach((k,v)->c.assets.put(k,v.copy()));c.fonts.clear();fonts.forEach((k,v)->c.fonts.put(k,v.copy()));
    ...
    public static final class TileDef {
        ...
        public boolean solid(){return !walkable;} public TileDef copy(){TileDef c=new TileDef(id,name,color,walkable,assetKey);c.oneWay=oneWay;c.friction=friction;c.damage=damage;c.animationClip=animationClip;c.tags.addAll(tags);return c;} @Override public String toString(){return name;}
    }

    public static final class Asset {
        public final String key; public String sourceName; public byte[] data; public boolean sourceOnly; public String sourceAssetKey=""; public int regionX,regionY,regionWidth,regionHeight; public FilterMode filterMode=FilterMode.PIXEL; public AssetCategory category=AssetCategory.IMAGE; public double pivotX=.5,pivotY=.5; public String sourcePath=""; public long sourceModified; public final LinkedHashSet<String>tags=new LinkedHashSet<>(); private transient BufferedImage image;
    ...
    public static final class AnimatorController {public final String key;public String name,defaultState="";public final LinkedHashMap<String,AnimatorState>states=new LinkedHashMap<>();public final List<AnimatorTransition>transitions=new ArrayList<>();public AnimatorController(String key,String name){this.key=key;this.name=name;}public AnimatorController copy(){AnimatorController c=new AnimatorController(key,name);c.defaultState=defaultState;states.forEach((k,v)->c.states.put(k,v.copy()));transitions.forEach(t->c.transitions.add(t.copy()));return c;}@Override public String toString(){return name;}}
    ...
    public static final class ParticlePreset {public final String key;public String name,assetKey="";public double rate=10,lifetime=1,speed=1,spread=45,gravity,startScale=1,endScale=.2,startOpacity=1,endOpacity=0;public boolean burst,localSpace=true;public int burstCount=12;public ParticlePreset(String key,String name){this.key=key;this.name=name;}public ParticlePreset copy(){ParticlePreset c=new ParticlePreset(key,name);c.assetKey=assetKey;c.rate=rate;c.lifetime=lifetime;c.speed=speed;c.spread=spread;c.gravity=gravity;c.startScale=startScale;c.endScale=endScale;c.startOpacity=startOpacity;c.endOpacity=endOpacity;c.burst=burst;c.localSpace=localSpace;c.burstCount=burstCount;return c;}@Override public String toString(){return name;}}
    ...
    public static final class ComponentDef {public final String type;public final LinkedHashMap<String,String>properties=new LinkedHashMap<>();public ComponentDef(String type){this.type=type;}public ComponentDef(String type,Map<String,String>d){this.type=type;properties.putAll(d);}public ComponentDef copy(){return new ComponentDef(type,properties);}public String get(String k,String d){return properties.getOrDefault(k,d);}public double number(String k,double d){try{return Double.parseDouble(get(k,Double.toString(d)));}catch(Exception e){return d;}}public boolean bool(String k,boolean d){return Boolean.parseBoolean(get(k,Boolean.toString(d)));}
        public static ComponentDef preset(String type){LinkedHashMap<String,String>p=new LinkedHashMap<>();p.put("enabled","true");switch(type){
            ...
            case"Camera2D"->{p.put("target","");p.put("follow","true");p.put("zoom","1");p.put("offsetX","0");p.put("offsetY","0");p.put("pixelSnap","true");p.put("priority","0");}
